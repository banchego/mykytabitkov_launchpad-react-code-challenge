...
function Dropdown(props) {
  ...
  return (
    <div>
      <ul className="dropdown">{mappedOptions}</ul>
    </div>
  );
}
...
function UniList(props) {
  const mappedUnis = props.options.map((ele, index) => {
    return (
      <li className="uni-element" key={index}>
        {ele.name}
      </li>
    );
  });

  return <div className="uni-list">{mappedUnis}</div>;
}
...
export default function Universities() {
  ...
  function update() {
    fetch(URL_UNI_INFO + "search?country=" + selection)
      .then((response) => response.json())
      .then((json) => setUniversities(json));
  }

  useEffect(() => {
    fetch(URL_COUNTRIES_INFO)
      .then((response) => response.json())
      .then((json) => json["data"])
      .then((res) => {
        setCountries(res);
  ...
  return (
    <div>
      ...
      {menuIsOpen && (
        <Dropdown options={countries} assignSelection={assignSelection} />
      )}
